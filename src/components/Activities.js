...
const CallBox = ({
  ...
  const handleCallArchive = () => {
    ...
    fetch(`https://charming-bat-singlet.cyclic.app/https://cerulean-marlin-wig.cyclic.app/${id}`, requestOptions)
...
const ArchiveOrUndo = ({
  ...
  const handleArchiveAll = () => {
    activities.forEach((call, callIndex) => {
      ...
      fetch(
        `https://charming-bat-singlet.cyclic.app/https://cerulean-marlin-wig.cyclic.app/${call.id}`,
        requestOptions
      )
        .then((response) => response.json())
        .then((json) => {
