import {
  ...
  DotStyle,
  DotContainer
} from "../ControlBar/ControlBarStyles";
...
export default function ControlBar(activities) {
  ...
  const realActicities = (activities.activities && activities.activities.length > 0) ? 
                          activities.activities.filter((val) => val.via && val.to && val.from && val.to !== val.from && val.is_archived == false) : 
                          [];
  ...
  return (
    <ControlBarStyle>
      ...
      >
        {
          realActicities.length > 0 && (
            <div>{realActicities.length}</div>
          )
        }
        <PhoneStyle />
      </PhoneContainer>
